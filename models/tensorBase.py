...
class SineLayer(torch.nn.Module):
    ...
    def forward(self, input):
        # adding input like in RES-NET
        if self.is_first:
            return torch.sin(self.omega_0 * self.linear(input))

        return (torch.sin(self.omega_0 * self.linear(input)) + input) * 0.5
...
class MLPRender_SIREN(torch.nn.Module):
    def __init__(self, inChanel, viewpe=0, pospe=0, featureC=32):
        super(MLPRender_SIREN, self).__init__()
    ...
    def forward(self, pts, viewdirs, features):
        mlp_in = torch.cat([features, viewdirs], dim=-1)
        rgb = self.mlp(mlp_in)
        rgb = torch.sigmoid(rgb)
...
class TensorBase(torch.nn.Module):
    ...
    def init_render_func(self, shadingMode, pos_pe, view_pe, fea_pe, featureC, device):
        if shadingMode == 'MLP_SIREN':
            self.renderModule = MLPRender_SIREN(self.app_dim, view_pe, pos_pe, featureC).to(device)
        elif shadingMode == 'MLP_PE':
